...
test("openapi declares free reads with security: []", () => {
  const doc = buildOpenApi(env) as {
    paths: Record<string, { get?: { security?: unknown[]; "x-payment-info"?: unknown } }>;
  };
  const freeGets = ["/health", "/_meta", "/", "/feed", "/feed.xml", "/feed.json", "/llms", "/llms.txt", "/social", "/blog", "/blog/{site}/{slug}", "/numetal/fees", "/numetal/burns", "/numetal/status", "/agent"];
